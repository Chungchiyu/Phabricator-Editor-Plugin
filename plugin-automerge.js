...
#_PHE_MM_TIP{
  ...
  /* ════════════════════════════════════════════════════════════
     SCROLL SYNC — simple ratio
     ════════════════════════════════════════════════════════════ */
  function ScrollSyncer(ta, preview) {
    var self = this;
    var editorDriving = false, previewDriving = false;
    var edLock = null, pvLock = null;
    var LOCK_MS = 80;
    ta.addEventListener('scroll', function () {
      if (previewDriving) return;
    ...
    preview.addEventListener('scroll', function () {
      if (editorDriving) return;
      previewDriving = true; clearTimeout(pvLock);
      pvLock = setTimeout(function () { previewDriving = false; }, LOCK_MS);
      var r = preview.scrollTop / Math.max(1, preview.scrollHeight - preview.clientHeight);
      ta.scrollTop = r * (ta.scrollHeight - ta.clientHeight);
    });
    self.invalidate = function () { };
    self.destroy = function () { clearTimeout(edLock); clearTimeout(pvLock); };
  }
  ...
  function rebuildMinimap() {
    ...
    hs.forEach(function (h) {
      ...
      item.className = 'phe-mm-item ' + h.tagName.toLowerCase();
      item.addEventListener('mouseenter', function () {
        var r = item.getBoundingClientRect();
        MM_TIP.textContent = h.textContent;
  ...
  function updateMinimapActive() {
    if (!$.previewEl || !_mmHeadings.length) return;
    var st = $.previewEl.scrollTop + 60, active = 0;
  ...
  function startMinimap() {
    ...
    MM._up.onclick = function () { $.previewEl && $.previewEl.scrollTo({ top: 0, behavior: 'smooth' }); };
    MM._dn.onclick = function () { $.previewEl && $.previewEl.scrollTo({ top: $.previewEl.scrollHeight, behavior: 'smooth' }); };
    if ($.previewEl) {
  ...
  async function doAutoMerge() {
    ...
    try {
      ...
      if (!stA) { alert('❌ Failed to parse'); return; }
      var THEIRS = stA.value;
  ...
  document.getElementById('_PHE_SAVE').addEventListener('click', function () {
    doAutoMerge();
  });
  ...
  document.getElementById('_PHE_EDIT').addEventListener('click', function () {
    if (!$.active) {
      ...
      if (ta) {
        ...
        $.syncer = new ScrollSyncer(ta, preview);
      }
      $.active = true;
      startMinimap();
