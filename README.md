...
1. Make the bookmarks bar visible in your browser. In Chrome: click the three-dot menu → Bookmarks → Show bookmarks bar.
2. Right-click on the bookmarks bar → Add page. Paste the content of `plugin-inline.js` into the URL field.
3. Done! Click the bookmark to activate the plugin.

## File Structure
| File | Description |
|------|-------------|
| `plugin.js` | Source code (readable) |
| `plugin-inline.js` | Bookmarklet version (auto-generated by `convert.py`) |
| `convert.py` | Conversion script: `plugin.js` → `plugin-inline.js` |
| `assets/` | Static assets (screenshots, etc.) |

## Development
1. Edit `plugin.js`.
2. Run the conversion script to generate the bookmarklet:
    ```bash
    python3 convert.py
    ```
3. Paste the content of `plugin-inline.js` into a bookmark URL field to test.